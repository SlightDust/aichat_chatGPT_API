...
0. @bot+wxyy闭嘴：禁用群AI(临时会话不会受影响)
1. wxyy调整AI概率+{0-50之间的整数}: 调整群AI随机触发的概率
2. wxyy当前AI概率：查看群AI随机触发的概率
3. wxyy清空群设定：清空群AI保存的所有人格设定和对话
4. wxyy清空群对话：清空群AI保存的所有对话
5. wxyy查看本群token：当前群AI消耗的token，清空群对话会将token清零
6. wxyy添加群设定+{输入你的调教指令}：给群AI添加调教指令
7. wxyy创建临时会话：创建一个不受群设定影响的临时ERNIE-bot会话，创建后ERNIE-bot会回复你在群里的所有消息，一分钟内没收到消息会自动结束
8. wxyy继续临时会话：继续你上次的临时ERNIE-bot会话
9. wxyy结束临时会话：提前结束你的临时ERNIE-bot会话
10. /t + 内容：用前缀"/t"触发群AI，可在setting.py里修改前缀
11. wxyy查看群设定
12. （这个不行）调整上限：调整群AI每句话使用的token
13. wxyy调整记忆：调整群AI能记住的群消息
14. wxyy看看胖次：查看群AI的所有信息
15. wxyy启动/禁用语音：ATRI语音
"""
    }
    def __init__(self):
        self.load_config()
